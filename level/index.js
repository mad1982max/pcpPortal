...
function defineData4Floor() {
  ...
  console.log(
    "pointsOnLevel:",
    pointsOnLevel.map((item) => item.name)
  );
}
  ...
  if (levelInfo.isSub) {
    ...
    asideSvg.onload = function () {
      ...
      floorRect.forEach((singleBlock) => {
        ...
        singleBlock.addEventListener("mouseenter", (e) =>
          hoverColorizeSubFloors(e, true)
        );
        singleBlock.addEventListener("mouseleave", (e) =>
          hoverColorizeSubFloors(e)
        );
      });
    };
  } else {
...
function buildSvg() {
  ...
  floorLayer.attr("class", "floorLayer").on("click", () => {
...
function drawSet(currentSet, currentClusterData, setName = "set") {
  ...
  set
    ...
    .append("circle")
    .attr("fill", (d) =>
      setName === "set" && d.pointsCopy.length > 1
        ? clusterPinColor
        : singlePinColor
    )
    ...
    .attr("r", (d) =>
      setName === "set" && d.pointsCopy.length > 1
        ? currentClusterData.rCuster
        : currentClusterData.rPin
    )
  ...
  set
    ...
    .attr("font-size", (d) =>
      setName === "set" && d.pointsCopy.length > 1
        ? currentClusterData.textCluster
        : currentClusterData.textPin
    )
    .attr("fill", "white")
    .attr("font-family", "sans-serif")
    .attr("dy", (d) =>
      setName === "set" && d.pointsCopy.length > 1
        ? currentClusterData.dyTextCluster
        : currentClusterData.dyTextPin
    )
    .attr("dx", (d) =>
      setName === "set" ? (d.pointsCopy.length > 1 ? -1 : 0) : -1
    )
    .attr("pointer-events", "none")
    .text((d) =>
      setName === "set"
        ? d.pointsCopy.length > 1
          ? d.pointsCopy.length
          : d.pointsCopy[0].name
        : d.name
    );
